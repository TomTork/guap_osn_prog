...
pair<int, vector<pair<int, int>>> LifeTimeOfBacterium(int N, int K1, int K2, int D, int T, int P){
    ...
    vector<pair<int, int>> answerCoords;
    int i, j;
    int answerDay = -1;
    while (true){
        ...
        if(day < D){
            ...
            for(i = 0; i < k; i++){
                ...
                answerCoords.push_back( { randomCoords.first, randomCoords.second } );
    ...
    return pair<int, vector<pair<int, int>>> { answerDay, answerCoords };
}
...
int main(){
    ...
    vector<pair<int, vector<pair<int, int>>>> answer;
    for(int i = 0; i < 100; i++){
        answer.push_back(LifeTimeOfBacterium(200, 20, 20, 1, __INT_MAX__, 100));
    }
    for(int i = 0; i < answer.size(); i++){
        for(int j = i; j < answer.size(); j++){
            if(answer[i].first < answer[j].first) swap(answer[i], answer[j]);
        }
    }
    for(auto e: answer){
        cout << e.first << endl;
